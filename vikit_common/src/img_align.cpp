 ...
 */
ForwardCompositionalSE3::ForwardCompositionalSE3(
    vector<PinholeCamera> &cam_pyr, vector<cv::Mat> &depth_pyr,
    vector<cv::Mat> &img_pyr, vector<cv::Mat> &tpl_pyr,
    vector<cv::Mat> &img_pyr_dx, vector<cv::Mat> &img_pyr_dy, SE3 &init_model,
    int n_levels, int n_iter, float res_thresh, bool display, Method method,
    int test_id)
    : cam_pyr_(cam_pyr), depth_pyr_(depth_pyr), img_pyr_(img_pyr),
      tpl_pyr_(tpl_pyr), img_pyr_dx_(img_pyr_dx), img_pyr_dy_(img_pyr_dy),
      display_(display), log_(test_id < 0), res_thresh_(res_thresh) {
  n_iter_ = n_iter;
  method_ = method;
...
ForwardCompositionalSE3::ForwardCompositionalSE3(
    vector<PinholeCamera> &cam_pyr, vector<cv::Mat> &depth_pyr,
    vector<cv::Mat> &img_pyr, vector<cv::Mat> &tpl_pyr,
    vector<cv::Mat> &img_pyr_dx, vector<cv::Mat> &img_pyr_dy, int n_levels,
    int n_iter, float res_thresh, bool display, Method method, int test_id)
    : cam_pyr_(cam_pyr), depth_pyr_(depth_pyr), img_pyr_(img_pyr),
      tpl_pyr_(tpl_pyr), img_pyr_dx_(img_pyr_dx), img_pyr_dy_(img_pyr_dy),
      display_(display), log_(test_id < 0), res_thresh_(res_thresh) {
  n_iter_ = n_iter;
...
void ForwardCompositionalSE3::runOptimization(SE3 &model, int levelBegin,
                                              int levelEnd) {
  ...
  for (level_ = levelBegin; level_ >= levelEnd; --level_) {
    mu_ = 0.1;
    cout << endl
         << "PYRAMID LEVEL " << level_ << endl
         << "---------------" << endl;
    optimize(model);
  }
}

double ForwardCompositionalSE3::computeResiduals(const SE3 &model,
  ...
  for (int v = 0; v < depth_pyr_[level_].rows; ++v) {
    for (int u = 0; u < depth_pyr_[level_].cols; ++u) {
      ...
      Vector2f uv_img_pyr =
          cam_pyr_[level_].world2cam(xyz_img).cast<float>(); // apply cam model
      if (cam_pyr_[level_].isInFrame(uv_img_pyr.cast<int>(), 2)) {
        ...
        if (linearize_system) {
          ...
          float dx = 0.5 * interpolateMat_32f(img_pyr_dx_[level_],
                                              uv_img_pyr[0], uv_img_pyr[1]);
          float dy = 0.5 * interpolateMat_32f(img_pyr_dy_[level_],
                                              uv_img_pyr[0], uv_img_pyr[1]);
...
int ForwardCompositionalSE3::solve() {
...
void ForwardCompositionalSE3::update(const ModelType &old_model,
                                     ModelType &new_model) {
 ...
 */
SecondOrderMinimisationSE3::SecondOrderMinimisationSE3(
    vector<PinholeCamera> &cam_pyr, vector<cv::Mat> &depth_pyr,
    vector<cv::Mat> &img_pyr, vector<cv::Mat> &tpl_pyr,
    vector<cv::Mat> &img_pyr_dx, vector<cv::Mat> &img_pyr_dy,
    vector<cv::Mat> &tpl_pyr_dx, vector<cv::Mat> &tpl_pyr_dy, SE3 &init_model,
    int n_levels, int n_iter, float res_thresh, bool display, Method method,
    int test_id)
    : cam_pyr_(cam_pyr), depth_pyr_(depth_pyr), img_pyr_(img_pyr),
      tpl_pyr_(tpl_pyr), img_pyr_dx_(img_pyr_dx), img_pyr_dy_(img_pyr_dy),
      tpl_pyr_dx_(tpl_pyr_dx), tpl_pyr_dy_(tpl_pyr_dy), display_(display),
      log_(test_id < 0), res_thresh_(res_thresh) {
  n_iter_ = n_iter;
  method_ = method;
  verbose_ = false;
  ...
  // level_ = n_levels-1;
  {
    ...
    if (display_) {
      cout << endl
           << "PYRAMID LEVEL " << level_ << endl
           << "patch-width = " << img_pyr_[level_].cols << endl
           << "---------------" << endl;
    }
    optimize(init_model);
  }
...
double SecondOrderMinimisationSE3::computeResiduals(const SE3 &model,
                                                    bool linearize_system,
                                                    bool compute_weight_scale) {
  // Warp the image such that it aligns with the template image
  double chi2 = 0;
  ...
  // Compute Warp
  cv::Mat mask =
      cv::Mat_<bool>(tpl_pyr_[level_].rows, tpl_pyr_[level_].cols, false);
  cv::Mat img_warped =
      cv::Mat_<float>(tpl_pyr_[level_].rows, tpl_pyr_[level_].cols, 1.0);
  ...
  for (int v = 0; v < depth_pyr_[level_].rows; ++v) {
    for (int u = 0; u < depth_pyr_[level_].cols; ++u) {
      if (mask.at<bool>(v, u)) {
        ...
        if (linearize_system) {
          // 0.25 because we have two 0.5 factors. First from adding the two
          // gradients and the second when we compute the sobel mask
          float dx = 0.25 * (tpl_pyr_dx_[level_].at<float>(v, u) +
                             img_warped_dx.at<float>(v, u));
          float dy = 0.25 * (tpl_pyr_dy_[level_].at<float>(v, u) +
                             img_warped_dy.at<float>(v, u));
...
void SecondOrderMinimisationSE3::update(const ModelType &old_model,
                                        ModelType &new_model) {
  new_model = SE3::exp(x_) * old_model;
}
